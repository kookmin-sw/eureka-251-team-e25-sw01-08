    ...
    - **🗓️ 4주차 (09/26)**  
      ...
      > **MIT Sixth Sense 연구팀이 만들고자 했던 것은?**  
        * Why?
          ...
          ```
          - 디지털 정보가 화면 속에 갇혀 있어 현실의 맥락과 단절됨 → “세상 그 자체를 인터페이스”로 만들어, 일상적인 사물과 행동 속에서 정보를 자연스럽게 쓰기 위함
          ```

        * What?
          
          ```
          - 목걸이처럼 착용하는 웨어러블 제스처 인터페이스. 카메라가 손짓과 사물을 인식하고, 포켓 프로젝터가 정보(지도, 사진, 문자 등)를 벽/손바닥/책 위에 바로 투영하여 상호작용하게 함
          ```
          
        * How?
          
          ```
          - 카메라 + 프로젝터 + 거울 + 모바일 컴퓨팅 + 손가락의 컬러 마커로 컴퓨터 비전 기반 제스처 추적 
          ```
      ...
      > **As-Is vs To-Be는 어떻게? 기존 컴퓨터의 입력, 출력 장치를 어떠한 방식으로 변경하였나요??**  
        * 키보드는?
          ...
          ```
          - 키보드: 물리 키 입력 → 손바닥/벽에 투영된 가상 키패드 터치
          ```
        * 마우스는?
          
          ```
          - 마우스: 포인팅·클릭 → 손 제스처(핀치로 확대/축소, 스와이프로 넘김)
          ```
        * 모니터는?

          ```
          - 모니터: 고정된 화면 → 어디든 디스플레이(신문, 종이, 손, 벽 등 현실 표면)
          ```
